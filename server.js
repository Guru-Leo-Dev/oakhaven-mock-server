...
const ADJECTIVES = ['Oakview', 'Harbor', 'Linden', 'Bramble', 'Meadow', 'Alder', 'Coastal', 'Willow'];
const NOUNS = {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (path.match(/^\/api\/products\/category\/[^/]+$/) && req.method === 'GET') {
    ...
    if (path === '/api/orders' && req.method === 'POST') {
      ...
      if (items.length === 0) {
        return sendJson(res, 400, { message: 'Order must include at least one item.' });
      }
    ...
    return sendJson(res, 404, { message: `No mock route for ${req.method} ${path}` });
  } catch (err) {
    return sendJson(res, 500, { message: 'Mock server error', detail: err.message });
  }
});
